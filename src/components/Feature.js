import React, { Component } from 'react';

class Feature extends Component {
    render() {
        return (
            <div>
                <div class="features">
                    <div class="container">
                            <div class="row" style={{margintop: '90px', marginbottom: '30px', color: '#ADADAD'}}>
                                <div class="col-md-2"></div>
                                <div class="col-md-5">
                                    <h2 style={{fontsize: '2.0rem'}}>FEATURES.</h2>
                                </div>
                                <div class="col-md-5"></div>
                            </div>
                        
                            <div class="row">
                                <div class="col-md-2"></div>
                                <div class="col-md-5 trai">
                                    <h4 class="features-title">24/7</h4>
                                    <div class="duoi" style={{display: 'flex'}}>
                                        <div class="col-md-2" style={{display: 'flex', flexdirection: 'column'}}>
                                            <div style={{backgroundcolor: '#15cda8', width: '52px', height: '3px'}}></div>    
                                            <div>24/7</div>
                                        </div>
                                        <div class="col-md-10">
                                            <p> Silver is an eco-friendly antiseptic used by people from ancient times. State-of-the-art technologies make it possible to use this metal in the form of tiny little particles. They are able to make an antibacterial that effects 1000 times more than the same amount of a massive metal. EVOLUT products eradicate more than 650 bacteria
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-5 phai">
                                    <h4 class="features-title">FAST</h4>
                                    <div class="duoi" style={{display: 'flex'}}>
                                        <div class="col-md-2" style={{display: 'flex', flexdirection: 'column'}}>
                                            <div style={{backgroundcolor: '#15cda8', width: '52px', height: '3px'}}></div>    
                                            <div>24/7</div>
                                        </div>
                                        <div class="col-md-10">
                                            <p> Silver is an eco-friendly antiseptic used by people from ancient times. State-of-the-art technologies make it possible to use this metal in the form of tiny little particles. They are able to make an antibacterial that effects 1000 times more than the same amount of a massive metal. EVOLUT products eradicate more than 650 bacteria
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-2"></div>
                                <div class="col-md-5 trai">
                                    <h4 class="features-title">Friendly</h4>
                                    <div class="duoi" style={{display: 'flex'}}>
                                        <div class="col-md-2" style={{display: 'flex', flexdirection: 'column'}}>
                                            <div style={{backgroundcolor: '#15cda8', width: '52px', height: '3px'}}></div>    
                                            <div>24/7</div>
                                        </div>
                                        <div class="col-md-10">
                                            <p> Silver is an eco-friendly antiseptic used by people from ancient times. State-of-the-art technologies make it possible to use this metal in the form of tiny little particles. They are able to make an antibacterial that effects 1000 times more than the same amount of a massive metal. EVOLUT products eradicate more than 650 bacteria
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-5 phai">
                                    <h4 class="features-title">AFFORDABLE</h4>
                                    <div class="duoi" style={{display: 'flex'}}>
                                        <div class="col-md-2" style={{display: 'flex', flexdirection: 'column'}}>
                                            <div style={{backgroundcolor: '#15cda8', width: '52px', height: '3px'}}></div>    
                                            <div>24/7</div>
                                        </div>
                                        <div class="col-md-10">
                                            <p> Silver is an eco-friendly antiseptic used by people from ancient times. State-of-the-art technologies make it possible to use this metal in the form of tiny little particles. They are able to make an antibacterial that effects 1000 times more than the same amount of a massive metal. EVOLUT products eradicate more than 650 bacteria
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        </div>
                    </div>
                </div>
            </div>
        );
    }
}

export default Feature;